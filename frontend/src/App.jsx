...
function getNeuroPorNome(nome) {
  return NEURO_DATA[nome] || { cerebro: 'Sistema Límbico + Córtex Visual', reacao: 'Resposta emocional calibrada ao estilo', gatilho: 'Combinação de cor, ritmo e composição ativa resposta emocional específica', janela: '1–4s', retencao: '85%', hormonio: 'Dopamina', tecnica: 'Linguagem visual neurológica — cada elemento foi escolhido para ativar uma emoção específica' }
}
...
function EfeitoPsicologico({ preset, glow }) {
  ...
  useEffect(() => {
    setVisivel(false); setDigitando(true); setTextoAtual('')
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (i <= textoCompleto.length) { setTextoAtual(textoCompleto.slice(0, i)); i++ }
      else { setDigitando(false); clearInterval(interval) }
    }, 18)
  ...
  return (
    <div style={{ animation: 'fadeUp 0.6s ease both' }}>
      ...
      <div style={{ padding: '20px 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
        {[{ icon: '🧠', label: 'Área Ativada', valor: neuro.cerebro }, { icon: '⚡', label: 'Reação', valor: neuro.reacao }, { icon: '🧪', label: 'Hormônio', valor: neuro.hormonio }, { icon: '⏱', label: 'Janela de Impacto', valor: neuro.janela }].map(item => (
...
const TEMPLATES = [
  ...
  { label: '🌙 Noir misterioso', prompt: 'suspense dark noir thriller crime sombrio' },
  { label: '💜 Cyberpunk', prompt: 'cyberpunk neon futurista tech glitch matrix' },
  { label: '🌹 Romance', prompt: 'romance amor suave intimidade saudade delicado' },
  { label: '⚔️ Épico', prompt: 'épico guerra herói grandioso batalha destino' },
...
function VariacoesKit({ emocao, onSelecionar, glow }) {
  const lower = emocao.toLowerCase()
  const matches = Object.values(PRESETS).filter(p => p.keywords.some(k => lower.includes(k))).slice(0, 3)
  if (matches.length < 2) return null
  return (
    <div style={{ marginBottom: '20px', animation: 'fadeUp 0.4s ease both' }}>
      <p style={{ fontSize: '0.6rem', letterSpacing: '3px', color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase', marginBottom: '12px' }}>🎨 Variações detectadas — escolha uma:</p>
  ...
  return (
    <div style={{ position: 'fixed', bottom: '32px', left: '50%', transform: 'translateX(-50%)', zIndex: 1000, animation: 'fadeUp 0.4s ease both', background: 'rgba(10,10,20,0.95)', border: '1px solid rgba(124,58,237,0.4)', borderRadius: '20px', padding: '20px 24px', backdropFilter: 'blur(20px)', minWidth: '320px', maxWidth: '480px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
        ...
        <button onClick={copy} style={{ padding: '10px 16px', borderRadius: '10px', border: 'none', background: copied ? 'rgba(0,255,150,0.2)' : 'linear-gradient(135deg, #7c3aed, #e94560)', color: '#fff', fontSize: '0.8rem', fontWeight: '700', cursor: 'pointer', whiteSpace: 'nowrap' }}>{copied ? '✓ Copiado!' : 'Copiar'}</button>
      </div>
    </div>
  )
}
...
function PaginaKitPublico({ shareId }) {
  ...
  useEffect(() => {
    fetch(`${API}/kit/${shareId}`).then(r => r.json()).then(data => { setKit(data); setLoading(false) }).catch(() => { setErro(true); setLoading(false) })
  }, [shareId])
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '80px 28px 60px', position: 'relative', zIndex: 2 }}>
        ...
        {aba === 'gerar' && (
          <div style={{ animation: 'fadeUp 0.5s ease both' }}>
            ...
            {/* Variações */}
            {emocao.length > 3 && (
              <VariacoesKit emocao={emocao} glow={active?.glow || '#7c3aed'}
                onSelecionar={(p) => { setPreview(p); setEmocao(p.keywords[0]) }}
              />
            )}
